...
(function (factory) {
  ...
  Mapael.prototype = {
    ...
    init: function () {
      ...
      if ($[pluginName] && $[pluginName].maps && $[pluginName].maps[self.options.map.name]) {
        ...
        if (globalThis.console && globalThis.console.warn) {
          globalThis.console.warn(`Extending $.fn.mapael is deprecated (map '${  self.options.map.name  }')`)
        }
      } else {
    ...
    destroy: function () {
      ...
      if (self.onResizeEvent) $(globalThis).off(`resize.${  pluginName}`, self.onResizeEvent)
    ...
    initResponsiveSize: function () {
      ...
      $(globalThis).on(`resize.${  pluginName}`, self.onResizeEvent)
    ...
    onZoomEvent: function (e, zoomOptions) {
      ...
      var animDuration = (zoomOptions.animDuration === undefined) ? self.options.map.zoom.animDuration : zoomOptions.animDuration
      ...
      if (zoomOptions.area === undefined) {
        ...
        var areaMargin = (zoomOptions.areaMargin === undefined) ? 10 : zoomOptions.areaMargin
    ...
    onUpdateEvent: function (e, opt) {
      ...
      var fnShowElement = function (elem) {
        ...
        self.setElementOpacity(
          elem,
          (elem.mapElem.originalAttrs.opacity === undefined) ? 1 : elem.mapElem.originalAttrs.opacity,
          animDuration
        )
      }
    ...
    drawLinksCollection: function (linksCollection) {
      ...
      $.each(linksCollection, function (id) {
        ...
        p1 = typeof linksCollection[id].between[0] === 'string' ? self.options.plots[linksCollection[id].between[0]] : linksCollection[id].between[0]

        p2 = typeof linksCollection[id].between[1] === 'string' ? self.options.plots[linksCollection[id].between[1]] : linksCollection[id].between[1]
    ...
    isAttrsChanged: function(originalAttrs, newAttrs) {
      for (var key in newAttrs) {
        if (newAttrs.hasOwnProperty(key) && originalAttrs[key] === undefined || newAttrs[key] !== originalAttrs[key]) {
          return true
        }
      }
      return false
    ...
    updateElem: function (elem, animDuration) {
      ...
      if (elem.textElem) {
        ...
        if (elem.options.size > 0 || (elem.options.width && elem.options.height)) {
    ...
    drawLegend: function (legendOptions, legendType, elems, scale, legendIndex) {
      ...
      for (i = 0; i < legendOptions.slices.length; ++i) {
        ...
        yCenterCurrent += legendType === "plot" && (sliceOptions[i].type === undefined || sliceOptions[i].type === "circle") ? scale * sliceOptions[i].attrs.r : scale * sliceOptions[i].attrs.height / 2
    ...
    handleClickOnLegendElem: function(elem, id, legendIndex, legendType, opts) {
      ...
      legendOptions = $.isArray(self.options.legend[legendType]) ? self.options.legend[legendType][legendIndex] : self.options.legend[legendType]
      ...
      var animDuration = opts.animDuration === undefined ? legendOptions.hideElemsOnClick.animDuration : opts.animDuration 
      ...
      $.each(mapElems, function (y) {
        ...
        elemValue = $.isArray(mapElems[y].options.value) ? mapElems[y].options.value[legendIndex] : mapElems[y].options.value
        ...
        if (self.getLegendSlice(elemValue, legendOptions) === sliceOptions) {
          if (hidden === '0') { // we want to hide this element
            ...
            if ($.isEmptyObject(hiddenBy)) {
              self.setElementOpacity(
                mapElems[y],
                mapElems[y].mapElem.originalAttrs.opacity === undefined ? 1 : mapElems[y].mapElem.originalAttrs.opacity,
                animDuration
              )
            }
          }
    ...
    setHoverOptions: function (elem, originalAttrs, attrsHover) {
      ...
      elem.originalAttrs = elem.attrsHover.transform ? $.extend({ transform: "s1" }, originalAttrs) : originalAttrs
    },
    ...
    elemEnter: function (elem) {
      ...
      if (elem.options && elem.options.tooltip !== undefined) {
        ...
        if (elem.options.tooltip.content !== undefined) {
          ...
          content = typeof elem.options.tooltip.content === "function" ? elem.options.tooltip.content(elem.mapElem) : elem.options.tooltip.content
        }
      ...
      if ((elem.mapElem !== undefined || elem.textElem !== undefined) && self.paper.safari) self.paper.safari()
    },
    ...
    elemHover: function (elem, event) {
      ...
      if (elem.options.tooltip !== undefined) {
        ...
        if (typeof elem.options.tooltip.offset === "object") {
          if (elem.options.tooltip.offset.left !== undefined) {
            offsetLeft = elem.options.tooltip.offset.left
          }
          if (elem.options.tooltip.offset.top !== undefined) {
    ...
    animateViewBox: function (targetX, targetY, targetW, targetH, duration, easingFunction) {
      ...
      self.zoomAnimStartTime = Date.now()
      ...
      var computeNextStep = function () {
        ...
        var elapsed = Date.now() - self.zoomAnimStartTime
        ...
        if (elapsed < durationWithMargin) {
          ...
          if (oldZoomAnimStartTime && self.zoomAnimCVBTarget && self.zoomAnimCVBTarget.w !== targetW) {
            ...
            var realElapsed = Date.now() - oldZoomAnimStartTime
    ...
    requestAnimationFrame: function(callback) {
      return this._requestAnimationFrameFn.call(globalThis, callback)
    },
    // The function to use for cancelAnimationFrame
    cancelAnimationFrame: function(id) {
      this._cancelAnimationFrameFn.call(globalThis, id)
    },
