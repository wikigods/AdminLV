...
(function( factory ){
  ...
  $.extend( KeyTable.prototype, {
    ...
    _constructor: function ()
    {
      ...
      if ( this.c.blurable ) {
        $( document ).on( `mousedown${namespace}`, function ( e ) {
          ...
          if ( $(e.target).parents( '.dataTables_filter' ).length > 0 ) {
            that._blur()
          }
          ...
          if (
            $(e.target).parents('div.editor-datetime').length > 0 ||
					$(e.target).parents('div.dt-datetime').length > 0 
          ) {
            return
          }

          //If the click was inside the fixed columns container, don't blur
          if ( $(e.target).parents().filter('.DTFC_Cloned').length > 0 ) {
            return
          }
    ...
    _editor: function ( key, orig, hardEdit )
    {
      ...
      if ( $('div.DTE', editCell.node()).length > 0 ) {
        return
      }
