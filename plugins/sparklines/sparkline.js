(function (root, factory) {
  ...
  function Sparkline(element, options) {
    ...
    init: {
      ...
      if (this.options.tooltip) {
        ...
        this.canvas.addEventListener('mousemove', showTooltip.bind(this))
      }
    }
  }
  ...
  function getY(minValue, maxValue, offsetY, height, index) {
    ...
    return range == 0 ? offsetY + height / 2 : (offsetY + height) - ((this[index] - minValue) / range) * height
  }
  ...
  function drawLine(x1, x2, style, x, y){
    ...
    this.context.beginPath()
    this.context.moveTo(style.direction == 'right' ? x : x1, y)
    this.context.lineTo(style.direction == 'left' ? x : x2, y)
    this.context.stroke()
  ...
  Sparkline.prototype.draw = function (points) {
    ...
    var bottomValue = this.options.minValue == undefined ? Math.min(minValue, this.options.maxMinValue == undefined ? minValue : this.options.maxMinValue) : this.options.minValue
    var topValue = this.options.maxValue == undefined ? Math.max(maxValue, this.options.minMaxValue == undefined ? maxValue : this.options.minMaxValue) : this.options.maxValue
    var minX = offsetX
    var maxX = offsetX
