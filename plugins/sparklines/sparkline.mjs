export default class Sparkline {
  ...
  draw(points = []) {
    ...
    const bottomValue = this.options.minValue == undefined ? Math.min(minValue, this.options.maxMinValue == undefined ? minValue : this.options.maxMinValue) : this.options.minValue
    const topValue = this.options.maxValue == undefined ? Math.max(maxValue, this.options.minMaxValue == undefined ? maxValue : this.options.minMaxValue) : this.options.maxValue
    let minX = offsetX
    let maxX = offsetX
    ...
    const line = (style, x, y) => {
      ...
      this.context.moveTo(style.direction == 'right' ? x : offsetX, y)
      this.context.lineTo(style.direction == 'left' ? x : width + offsetX, y)
      this.context.stroke()
