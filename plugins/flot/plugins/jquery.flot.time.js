...
(function($) {
  ...
  function dateGenerator(ts, opts) {
    var maxDateValue = 8_640_000_000_000_000
    ...
    } else if (typeof timezoneJS !== "undefined" && timezoneJS.Date !== undefined) {
  ...
  var timeUnitSizeMicroseconds = {
    ...
    "quarter": 3 * 30 * 24 * 60 * 60 * 1_000_000,
    "year": 365.2425 * 24 * 60 * 60 * 1_000_000
  }
  ...
  var specMonths = [...baseSpec, [3, "month"], [6, "month"],
    [1, "year"]]
  var specQuarters = [...baseSpec, [1, "quarter"], [2, "quarter"],
    [1, "year"]]
  ...
  function dateTickGenerator(axis) {
    ...
    if (opts.minTickSize !== null && opts.minTickSize !== undefined) {
      minSize = typeof opts.tickSize === "number" ? opts.tickSize : opts.minTickSize[0] * timeUnitSize[opts.minTickSize[1]]
    }
    ...
    if (unit === "year") {
      ...
      if (opts.minTickSize !== null && opts.minTickSize !== undefined && opts.minTickSize[1] === "year") {
        ...
        var magn = parseFloat(`1e${  Math.floor(Math.log10(axis.delta / timeUnitSize.year))}`)
          ...
          axis.tickFormatter = function (v, axis) {
            ...
            if (t < timeUnitSize.second) {
              ...
              if (t < timeUnitSize.minute) {
                ...
                fmt = span < 2 * timeUnitSize.day ? `${hourCode  }:%M${  suffix}` : `%b %d ${  hourCode  }:%M${  suffix}`
              } else if (t < timeUnitSize.month) {
                fmt = "%b %d"
              } else if ((useQuarters && t < timeUnitSize.quarter) ||
                            (!useQuarters && t < timeUnitSize.year)) {
                fmt = span < timeUnitSize.year ? "%b" : "%b %Y"
              } else if (useQuarters && t < timeUnitSize.year) {
                fmt = span < timeUnitSize.year ? "Q%q" : "Q%q %Y"
              } else {
                fmt = "%Y"
              }
