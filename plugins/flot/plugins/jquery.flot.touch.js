  ...
  function initTouchNavigation(plot, options) {
    ...
    var longTap = {
      ...
      isLongTap: function(e) {
        ...
        if (tapDuration >= minLongTapDuration && !gestureState.interceptedLongTap && distance(gestureState.currentTapStart.x, gestureState.currentTapStart.y, gestureState.currentTapEnd.x, gestureState.currentTapEnd.y) < maxLongTapDistance) {
    ...
    var tap = {
      touchstart: function(e) {
        gestureState.tapStartTime = Date.now()
      },
      ...
      isTap: function(e) {
        var currentTime = Date.now(),
          tapDuration = currentTime - gestureState.tapStartTime
        if (tapDuration <= pressedTapDuration && distance(gestureState.currentTapStart.x, gestureState.currentTapStart.y, gestureState.currentTapEnd.x, gestureState.currentTapEnd.y) < maxLongTapDistance) {
          return true
        }
        return false
      }
    ...
    function isDoubleTap(e) {
      ...
      if (intervalBetweenTaps >= 0 && intervalBetweenTaps < maxIntervalBetweenTaps && distance(gestureState.prevTap.x, gestureState.prevTap.y, gestureState.currentTap.x, gestureState.currentTap.y) < maxDistanceBetweenTaps) {
    ...
    function updateOnMultipleTouches(e) {
      gestureState.isUnsupportedGesture = e.touches.length >= 3 ? true : false
    }

    function isPinchEvent(e) {
      if (e.touches && e.touches.length >= 2 && e.touches[0].target === plot.getEventHolder() &&
