...
(function ($) {
  ...
  function computePreferedLogTickValues(endLimit, rangeStep) {
    var log10End = Math.floor(Math.log10(endLimit)) - 1,
      log10Start = -log10End,
      val, range, vals = []
  ...
  var clampAxis = function (axis, plot) {
    ...
    if (min <= 0) {
      ...
      min = axis.datamin === null ? (axis.min = 0.1) : processAxisOffset(plot, axis)

      if (max < min) {
        axis.max = axis.datamax === null ? axis.options.max : axis.datamax
  ...
  var logTickFormatter = function (value, axis, precision) {
    var tenExponent = value > 0 ? Math.floor(Math.log10(value)) : 0

    if (precision) {
      return (tenExponent >= -4) && (tenExponent <= 7) ? $.plot.defaultTickFormatter(value, axis, precision) : $.plot.expRepTickFormatter(value, axis, precision)
    }
  ...
  function setDataminRange(plot, axis) {
    if (axis.options.mode === 'log' && axis.datamin <= 0) {
      axis.datamin = axis.datamin === null ? 0.1 : processAxisOffset(plot, axis)
    }
  }
