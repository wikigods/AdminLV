...
(function () {
  ...
  var Criteria = /** @class */ (function () {
    function Criteria(table, opts, topGroup, index, depth) {
      ...
      $$2(globalThis).on('resize.dtsb', dataTable$2.util.throttle(function () {
        _this._adjustCriteria()
      }))
      this._buildCriteria()
      return this
    ...
    Criteria.prototype.getDetails = function (deFormatDates) {
      ...
      if (this.s.type.includes('num') && this.s.dt.page.info().serverSide) {
        for (var i = 0; i < this.s.value.length; i++) {
          this.s.value[i] = this.s.value[i].replaceAll(/[^0-9.]/g, '')
        }
      }
      return {
    ...
    Criteria.prototype._adjustCriteria = function () {
      ...
      var valRight
      var valWidth
      var outmostval = this.dom.value.at(-1)
    ...
    Criteria.prototype._clearValue = function () {
      if (this.s.condition === undefined) {
        ...
        this.dom.value = [this.s.conditions[this.s.condition].init(this, Criteria.updateListener)].flat()
        if (this.dom.value.length > 0 && this.dom.value[0] !== undefined) {
    ...
    Criteria.prototype._populateCondition = function () {
      ...
      if (conditionsLength === 0) {
        ...
        if (this.s.type.includes('moment')) {
          this.s.dateFormat = this.s.type.replaceAll('moment-', '')
        }
        else if (this.s.type.includes('luxon')) {
          this.s.dateFormat = this.s.type.replaceAll('luxon-', '')
        }
            ...
            if (!found) {
              ...
              var opt = {
                ...
                text: (col.searchBuilderTitle === undefined ?
                  col.sTitle :
                  col.searchBuilderTitle).replaceAll(/(<([^>]+)>)/ig, '')
              }
        ...
        var _loop_3 = function (data) {
          this_1.s.dt.columns().every(function (index) {
            ...
            if ((col.searchBuilderTitle === undefined ?
              col.sTitle :
              col.searchBuilderTitle).replaceAll(/(<([^>]+)>)/ig, '') === data.text) {
              data.index = index
              data.origData = col.data
            }
          })
          var newOpt = $$2('<option>', {
            text: data.text.replaceAll(/(<([^>]+)>)/ig, ''),
            value: data.index
          })
    ...
    Criteria.prototype._populateValue = function (loadedCriteria) {
      ...
      this.dom.value = [this.s.conditions[this.s.condition].init(this, Criteria.updateListener, loadedCriteria === undefined ? undefined : loadedCriteria.value)].flat()
    ...
    Criteria.initSelect = function (that, fn, preDefined, array) {
      ...
      options.sort(function (a, b) {
        switch (that.s.type) {
          ...
          case 'num': 
          case 'html-num': {
            if (+a.val().replaceAll(/(<([^>]+)>)/ig, '') < +b.val().replaceAll(/(<([^>]+)>)/ig, '')) {
              return -1
            }
            else if (+a.val().replaceAll(/(<([^>]+)>)/ig, '') > +b.val().replaceAll(/(<([^>]+)>)/ig, '')) {
              return 1
            }
            else {
              return 0
          ...
          case 'num-fmt': 
          case 'html-num-fmt': {
            if (+a.val().replaceAll(/[^0-9.]/g, '') < +b.val().replaceAll(/[^0-9.]/g, '')) {
              return -1
            }
            else if (+a.val().replaceAll(/[^0-9.]/g, '') > +b.val().replaceAll(/[^0-9.]/g, '')) {
              return 1
            }
            else {
              return 0
    ...
    Criteria.dateConditions = {
      ...
      'between': {
        ...
        search: function (value, comparison) {
          value = value.replaceAll(/(\/|-|,)/g, '-')
          return comparison[0] < comparison[1] ? comparison[0] <= value && value <= comparison[1] : comparison[1] <= value && value <= comparison[0]
        }
      },
    ...
    Criteria.momentDateConditions = {
      ...
      'between': {
        ...
        search: function (value, comparison, that) {
          ...
          return comp0 < comp1 ? comp0 <= val && val <= comp1 : comp1 <= val && val <= comp0
        }
      },
      ...
      if (!dataTable.DateTime) {
        ...
        if (types === undefined || types.includes(undefined) || types.includes(null)) {
          ...
          for (var _i = 0, _a = this.s.dt.settings()[0].aoColumns; _i < _a.length; _i++) {
            var colInit = _a[_i]
            types.push(colInit.searchBuilderType === undefined ? colInit.sType : colInit.searchBuilderType)
          }
        }
  ...
  (function (factory) {
    ...
    $(document).on('preInit.dt.dtsp', function (e, settings) {
      ...
      if ((settings.oInit.searchBuilder ||
                dataTable.defaults.searchBuilder) && !settings._searchBuilder) {
        _init(settings)
      }
    })
