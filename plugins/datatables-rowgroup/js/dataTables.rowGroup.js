...
(function( factory ){
  ...
  $.extend( RowGroup.prototype, {
    ...
    _groupDisplay: function ( level, groups )
    {
      ...
      for ( var i=0, ien=groups.length ; i<ien ; i++ ) {
        ...
        if ( this.c.endRender ) {
          ...
          if ( row ) {
            row.insertAfter( dt.row( rows.at( -1 ) ).node() )
          }
        }
    ...
    _rowWrap: function ( display, className, level )
    {
      ...
      else if (display instanceof $ && display.length > 0 && display[0].nodeName.toLowerCase() === 'tr') {
        row = display
      }
      else {
        row = $('<tr/>')
 ...
 */
  ...
  DataTable.Api.register( 'rowGroup().enabled()', function () {
    ...
    return ctx.length > 0 && ctx[0].rowGroup ?
