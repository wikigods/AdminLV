...
(function(factory) {
  ...
  function updateSelectionStates(dualListbox) {
    dualListbox.element.find('option').each(function(index, item) {
      var $item = $(item)
      if (($item.data('original-index')) === undefined) {
        $item.data('original-index', dualListbox.elementCount++)
      }
      if (($item.data('_selected')) === undefined) {
  ...
  function formatString(s, args) {
    console.log(s, args)
    return s.replaceAll(/{(\d+)}/g, function(match, number) {
  ...
  function filter(dualListbox, selectIndex) {
    ...
    options = selectIndex === 1 ? allOptions.not(':selected') : options.find('option:selected')
