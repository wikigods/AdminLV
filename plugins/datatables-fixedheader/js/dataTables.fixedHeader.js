 ...
 */
  $.extend( FixedHeader.prototype, {
    ...
    destroy: function () {
      ...
      if ( this.c.footer && this.dom.tfoot.length > 0 ) {
    ...
    update: function (force)
    {
      ...
      this._positions()
      this._scroll( force === undefined ? true : force )
    },
    ...
    _constructor: function ()
    {
      ...
      if ( ! this.c.headerOffset && autoHeader.length > 0 ) {
        this.c.headerOffset = autoHeader.outerHeight()
      }

      var autoFooter = $('.fh-fixedFooter')
      if ( ! this.c.footerOffset && autoFooter.length > 0 ) {
        ...
        el.find('th').each(function() {
          ...
          if ($(this).css('position') === 'sticky') {
            ...
            if (right !== 'auto' && !rtl) {
              ...
              var potential = +right.replaceAll('px', '') + (sign === '-' ? -1 : 1) * that.s.dt.settings()[0].oBrowser.barWidth
              $(this).css('right', Math.max(potential, 0))
            }
            else if(left !== 'auto' && rtl) {
              var potential = +left.replaceAll('px', '') + (sign === '-' ? -1 : 1) * that.s.dt.settings()[0].oBrowser.barWidth
              $(this).css('left', Math.max(potential, 0))
            }
          }
        })
      }
    ...
    _matchWidths: function ( from, to ) {
      var get = function ( name ) {
        return $(name, from)
				.map( function () {
				  return $(this).css('width').replaceAll(/[^\d\.]/g, '') * 1
				} ).toArray()
      }
    ...
    _modeChange: function ( mode, item, forceChange )
    {
      ...
      switch (mode) {
        ...
        // positioned floating table clone
          ...
          var val = this.c[`${item}Offset`] - (Math.max(shuffle, 0))
        ...
        case 'below': { // only used for the header
        ...
        case 'above': { // only used for the footer
 ...
 */
  ...
  $.each( ['header', 'footer'], function ( i, el ) {
    DataTable.Api.register( `fixedHeader.${el}Offset()`, function ( offset ) {
      ...
      if ( offset === undefined ) {
        return ctx.length > 0 && ctx[0]._fixedHeader ?
