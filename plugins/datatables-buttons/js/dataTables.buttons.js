  ...
  $.extend( Buttons.prototype, {
    ...
    add: function ( config, idx, draw )
    {
      ...
      this._expandButton(
        buttons,
        config,
        config === undefined ? undefined : config.split,
    ...
    _draw: function ( container, buttons )
    {
      ...
      for ( var i=0, ien=buttons.length ; i<ien ; i++ ) {
        ...
        if ( buttons[i].buttons && buttons[i].buttons.length > 0 ) {
    ...
    _expandButton: function ( attachTo, button, split, inCollection, inSplit, attachPoint, parentConf )
    {
      ...
      for ( var i=0, ien=buttons.length ; i<ien ; i++ ) {
        ...
        isSplit = conf.config !== undefined && conf.config.split ? true : false
    ...
    _buildButton: function ( config, inCollection, isSplit, inSplit )
    {
      ...
      var inserter
      inserter = buttonContainer && buttonContainer.tag ?
        $(`<${buttonContainer.tag}/>`)
    ...
    _popover: function ( content, hostButton, inOpts, e ) {
      ...
      if ( existingExpanded.length > 0 ) {
        ...
        if (hostNode.closest('div.dt-button-collection').length > 0) {
      ...
      setTimeout(function() {
        ...
        $('body')
				.on( 'click.dtb-collection', function (e) {
				  ...
				  if (( $(e.target).parents()[back]().filter( content ).length === 0  && !$(parent).hasClass('dt-buttons')) || $(e.target).hasClass('dt-button-background')) {
				    close()
				  }
				} )
 ...
 */
  Buttons.stripData = function ( str, config ) {
    ...
    str = str.replaceAll( /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '' )
  ...
  $.extend( _dtButtons, {
    collection: {
      ...
      action: function ( e, dt, button, config ) {
        if ( config._collection.parents('body').length > 0 ) {
 ...
 */
  ...
  DataTable.Api.register( 'buttons.info()', function ( title, message, time ) {
    ...
    if ( $('#datatables_buttons_info').length > 0 ) {
 ...
 */
  ...
  var _exportData = function ( dt, inOpts )
  {
    ...
    if ( dt.select && typeof dt.select.info === 'function' && modifier.selected === undefined  &&  dt.rows( config.rows, $.extend( { selected: true }, modifier ) ).any() ) {
