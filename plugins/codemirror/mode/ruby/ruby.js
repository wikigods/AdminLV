  ...
  CodeMirror.defineMode("ruby", function(config) {
    ...
    function tokenBase(stream, state) {
      ...
      if (ch == "`" || ch == "'" || ch == '"') {
        ...
        return regexpAhead(stream) ? chain(readQuoted(ch, "string-2", true), stream, state) : "operator"
      } else if (ch == "%") {
        ...
        return "operator"
      } else if (ch == "@" && /^@?[a-zA-Z_\u00A1-\uFFFF]/.test(stream)) {
        stream.eat("@")
        stream.eatWhile(/[\w\u00A1-\uFFFF]/)
        return "variable-2"
      } else if (ch == "$") {
    ...
    return {
      ...
      token: function(stream, state) {
        ...
        var style = state.tokenize.at(-1)(stream, state), kwtype
        var thisTok = curPunc
        if (style == "ident") {
      ...
      indent: function(state, textAfter) {
        if (state.tokenize.at(-1) != tokenBase) return CodeMirror.Pass
