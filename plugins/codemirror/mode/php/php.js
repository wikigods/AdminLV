...
(function(mod) {
  ...
  function phpString_(stream, state, closing, escapes) {
    ...
    // Simple syntax
    if (escapes !== false && /^\$[a-zA-Z_][a-zA-Z0-9_]*/.test(stream)) {
  ...
  var phpConfig = {
    ...
    hooks: {
      ...
      "}": function(_stream, state) {
        if (state.tokStack && state.tokStack.length > 0 &&
            !--state.tokStack[state.tokStack.length - 1]) {
          state.tokenize = phpString(state.tokStack.at(-2))
        }
        return false
      }
    }
  ...
  CodeMirror.defineMode("php", function(config, parserConfig) {
    ...
    function dispatch(stream, state) {
      ...
      if (!isPHP) {
        ...
        if (openPHP != -1) {
          state.pending = style == "string" && (m = cur.match(/[\'\"]$/)) && !/\?>/.test(cur) ? m[0] : { end: stream.pos, style: style }
    ...
    return {
      ...
      copyState: function(state) {
        ...
        cur = state.curMode == htmlMode ? htmlNew : phpNew
