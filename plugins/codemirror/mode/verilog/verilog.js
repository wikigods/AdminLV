  ...
  */
    ...
    var compilerDirectiveRegex      = new RegExp(
      `^(\`(?:ifdef|ifndef|elsif|else|endif|undef|undefineall|define|include|begin_keywords|celldefine|default|${ 
        String.raw`nettype|end_keywords|endcelldefine|line|nounconnected_drive|pragma|resetall|timescale|unconnected_drive))\b`}`)
    ...
    function tokenBase(stream, state) {
      ...
      if (ch == '`') {
        ...
        if (stream.eatWhile(/[\w\$_]/)) {
          ...
          if (cur.startsWith("`uvm_") && cur.endsWith("_begin")) {
            var keywordClose = `${curKeyword.slice(0,Math.max(0, curKeyword.length - 5))  }end`
      ...
      if (ch == '$') {
        stream.next()
        return stream.eatWhile(/[\w\$_]/) ? "meta" : null
      }
      // Time literals
      if (ch == '#') {
        stream.next()
      ...
      // Numeric literals
      if (realLiteral.test(stream) ||
        decimalLiteral.test(stream) ||
        binaryLiteral.test(stream) ||
        octLiteral.test(stream) ||
        hexLiteral.test(stream) ||
        unsignedNumber.test(stream) ||
        realLiteral.test(stream)) {
        return "number"
      }
    ...
    // the end of a line
      ...
      var re = new RegExp(`${String.raw`[{}()\[\]]|(` + allClosings.join("|")  })$`)
      return re
    }
    ...
    return {
      ...
      token: function(stream, state) {
        ...
        if (curPunc == ctx.type || isClosingAssignment) {
          ...
          } else if (curPunc == "}" && // Handle closing statements like constraint block: "foreach () {}" which
          // do not have semicolon at end.
          ctx && (ctx.type === "statement")) {
        ...
        } else if (curPunc == "newmacro" || (curKeyword && compilerDirectiveRegex.test(curKeyword))) {
          if (curPunc == "newmacro") {
          ...
          if (compilerDirectiveEndRegex.test(curKeyword)) {
            state.compilerDirectiveIndented -= statementIndentUnit
          }
          if (compilerDirectiveBeginRegex.test(curKeyword)) {
            state.compilerDirectiveIndented += statementIndentUnit
          }
        }
      ...
      indent: function(state, textAfter) {
        ...
        if (!compilerDirectivesUseRegularIndentation && compilerDirectiveRegex.test(textAfter)) {
          if (compilerDirectiveEndRegex.test(textAfter)) {
      ...
      token: function(stream, state) {
        ...
        if (stream.sol() && ! state.tlvInBlockComment) {
          ...
          if (stream.peek() == '\\') {
            style = "def"
            stream.skipToEnd()
            if (/\\SV/.test(stream.string)) {
              state.tlvCodeActive = false
            } else if (/\\TLV/.test(stream.string)){
