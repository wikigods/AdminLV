...
(function(mod) {
  ...
  CodeMirror.defineMode("soy", function(config) {
    ...
    function tokenUntil(stream, state, untilRegExp) {
      ...
      var match = untilRegExp.exec(oldString.slice(stream.pos))
      if (match) {
        // We don't use backUp because it backs up just the position, not the state.
        // This uses an undocumented API.
        stream.string = oldString.slice(0, Math.max(0, stream.pos + match.index))
      }
    ...
    function expression(stream, state) {
      ...
      if (/[[]/.test(stream)) {
        ...
        return null
      } else if (/\bmap(?=\()/.test(stream)) {
        state.soyState.push("map-literal")
        return "keyword"
      } else if (/\brecord(?=\()/.test(stream)) {
        state.soyState.push("record-literal")
        return "keyword"
      } else if (/([\w]+)(?=\()/.test(stream)) {
        ...
        return null
      } else if (/(null|true|false)(?!\w)/.test(stream) ||
          /0x([0-9a-fA-F]{2,})/.test(stream) ||
          /-?([0-9]*[.])?[0-9]+(e[0-9]*)?/.test(stream)) {
        return "atom"
      } else if (/(\||[+\-*\/%]|[=!]=|\?:|[<>]=?)/.test(stream)) {
    ...
    return {
      ...
      token: function(stream, state) {
        ...
        if (state.soyState.length === 0 || last(state.soyState) != "literal") {
          if (/^\/\*/.test(stream)) {
        ...
        switch (last(state.soyState)) {
          ...
          case "record-literal": {
            ...
            if (/[(,]/.test(stream)) {
              state.soyState.push("map-value", "record-key")
              return null
            }
            stream.next()
            return null
          ...
          case "map-literal": {
            ...
            if (/[(,]/.test(stream)) {
              state.soyState.push("map-value", "map-value")
              return null
            }
            stream.next()
            return null
          ...
          case "map-value": {
            if (stream.peek() == ")" || stream.peek() == "," || /^[:)]/.test(stream)) {
              state.soyState.pop()
              return null
            }
            return expression(stream, state)
          ...
          case "tag": {
            ...
            if (state.tag === undefined) {
              ...
              endTag = state.tag[0] == "/"
              tagName = endTag ? state.tag.slice(1) : state.tag
            }
        ...
        // A tag-keyword must be followed by whitespace, comment or a closing tag.
          ...
          var tagName = endTag ? state.tag.slice(1) : state.tag
          var tag = tags[tagName]
          if (state.tag != "/switch")
          ...
          var tagError = false
          if (tag) {
            if (!endTag && tag.soyState) state.soyState.push(tag.soyState)
        ...
        // Not a tag-keyword; it's an implicit print tag.
          ...
          state.soyState.push("tag")
          return "keyword"
        } else if (!state.context && stream.sol() && /import\b/.test(stream)) {
      ...
      innerMode: function(state) {
        return state.soyState.length > 0 && last(state.soyState) != "literal" ? null : last(state.localStates)
      },
  ...
  CodeMirror.registerHelper("hintWords", "soy", [...Object.keys(tags), "css", "debugger"])
