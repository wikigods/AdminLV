...
(function(mod) {
  ...
  CodeMirror.TernServer = function(options) {
    ...
    this.server = this.options.useWorker ?
  ...
  function findContext(doc, data) {
    ...
    while (cursor.findNext()) {
      var from = cursor.from(), dist = Math.abs(from.line - start.line) * 10_000
    ...
    if (before.length == 1)
      ...
      nearest = Pos(nearest.line + (before.length - 1), before.at(-1).length)
  ...
  function WorkerServer(ts) {
    ...
    worker.onmessage = function(e) {
      ...
      if (data.type == "getFile") {
        ...
        globalThis.console.log(data.message)
