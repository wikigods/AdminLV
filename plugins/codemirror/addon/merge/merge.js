...
(function(mod) {
  ...
  function alignableFor(cm, chunks, isOrig) {
    ...
    for (var i = 0;; i++) {
      var chunk = chunks[i]
      var chunkStart = chunk ? (isOrig ? chunk.origFrom : chunk.editFrom) : 1e9
  ...
  var MergeView = CodeMirror.MergeView = function(node, options) {
    ...
    CodeMirror.on(globalThis, "resize", onResize)
    var resizeInterval = setInterval(function() {
      for (var p = wrapElt.parentNode; p && p != document.body; p = p.parentNode) {}
      if (!p) { clearInterval(resizeInterval); CodeMirror.off(globalThis, "resize", onResize) }
    }, 5000)
  }
  ...
  function asString(obj) {
    return typeof obj == "string" ? obj : obj.getValue()
  }
  ...
  function getChunks(diff) {
    var chunks = []
    if (diff.length === 0) return chunks
